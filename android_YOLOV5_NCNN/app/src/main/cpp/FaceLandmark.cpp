...
FaceLandmark::FaceLandmark(AAssetManager *mgr) {

    FaceNet = new ncnn::Net();
    // opt 需要在加载前设置
//    FaceNet->opt.use_vulkan_compute = ncnn::get_gpu_count() > 0;  // gpu
...
//    LandmarkNet->opt.use_vulkan_compute = ncnn::get_gpu_count() > 0;  // gpu
